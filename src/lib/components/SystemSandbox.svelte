<script lang="ts">
  import { Copy, Check, Play, Terminal, Database, Sparkles, Layers, Users, Wind } from 'lucide-svelte';

  type ScriptTab = 'rustsale' | 'kelvot' | 'staffing' | 'lemyfinest';

  let activeTab = $state<ScriptTab>('rustsale');
  let copied = $state(false);
  let isExecuting = $state(false);
  let executionResult = $state<{
    traceId: string;
    events: { from: string; to: string; payload: string; status: string }[];
    metrics: { metric: string; value: string }[];
  } | null>(null);

  const scriptExamples = {
    rustsale: {
      filename: 'rustsale_omnichannel.rs',
      title: 'RustSale CRM — Động cơ Hội tụ Đa kênh (Rust Native)',
      code: `// RustSale CRM & Outreach Pro: Microsecond Event Loop
pub async fn handle_inbox_event(event: OmnichannelEvent) -> Result<SyncAck, EngineError> {
    match event {
        OmnichannelEvent::ZaloMessage { sender, text } => {
            keyring_vault::verify_signature(&sender)?;
            crm_pipeline::auto_tag_lead(&sender, "ZALO_INBOUND").await?;
            desktop_inbox::push_stream(Channel::Zalo, sender, text).await
        },
        OmnichannelEvent::WhatsAppB2B { phone, payload } => {
            crm_pipeline::link_won_contact(&phone, payload).await?;
            desktop_inbox::push_stream(Channel::WhatsApp, phone, payload).await
        },
        OmnichannelEvent::FacebookHub { page_id, comment } => {
            fanpage_ai::auto_reply(page_id, comment).await
        }
    }
}
// Độ trễ < 1ms, không rò rỉ bộ nhớ, bảo mật Local-First`,
      events: [
        { from: '@zalo:khach_hang_b2b', to: '@rustsale:inbox_hub', payload: 'Tin nhắn hội thoại mới', status: 'ĐÃ TIẾP NHẬN' },
        { from: '@whatsapp:doanh_nghiep_fdi', to: '@rustsale:won_contacts', payload: 'Đã gắn tag VIP Enterprise', status: 'ĐỒNG BỘ CRM' },
        { from: '@facebook:fanpage_chinh', to: '@rustsale:kanban_pipeline', payload: 'Tự động gán tư vấn viên', status: 'PHÂN QUYỀN XONG' }
      ],
      metrics: [
        { metric: '@rustsale:latency', value: '0.42 ms (Microsecond)' },
        { metric: '@rustsale:ram_usage', value: '38.4 MB (Tauri v2 Native)' },
        { metric: '@rustsale:security', value: 'AES-256 Keyring Hardware' }
      ]
    },
    kelvot: {
      filename: 'kelvot_bom_production.yaml',
      title: 'Kelvot ERP — Định mức BOM Sản xuất & Kho WMS',
      code: `kelvot_production_pipeline:
  order_id: "PO-2026-MFG-08"
  bom_version: "BOM_MOTOR_V3_MULTI_LEVEL"
  routing:
    - station_1: "Dập khuôn thép tấm (NVL: Thép lá cán nguội)"
    - station_2: "Gia công CNC & Quấn dây đồng tự động"
    - station_3: "Lắp ráp cụm chi tiết & Kiểm định QA/QC"
  accounting_sync:
    debit: "TK_154 (Chi phí sản xuất dở dang)"
    credit: "TK_152 (Nguyên vật liệu xuất kho WMS)"
    vas_standard: "Thong_tu_200_Bo_Tai_Chinh"`,
      events: [
        { from: '@kho_nguyen_lieu_wms', to: '@chuyen_san_xuat_1', payload: 'Thép tấm: 12.5 Tấn', status: 'XUẤT KHO FIFO' },
        { from: '@chuyen_san_xuat_1', to: '@kho_thanh_pham_wms', payload: 'Động cơ Diesel: 85 Bộ', status: 'QC ĐẠT 100%' },
        { from: '@tai_chinh:tk_154', to: '@ke_toan:gia_thanh_vas', payload: 'Giá thành: 450,000,000₫', status: 'GHI SỔ VAS' }
      ],
      metrics: [
        { metric: '@kelvot:bom_accuracy', value: '99.8% Chuẩn định mức' },
        { metric: '@kelvot:wms_barcode', value: 'Real-time Barcode Scan' },
        { metric: '@kelvot:accounting', value: 'VAS/IFRS Khép kín' }
      ]
    },
    staffing: {
      filename: 'it_staff_allocation.yaml',
      title: 'Cung Ứng Nhân Sự IT — Senior Onsite & Dedicated Squad',
      code: `it_staff_augmentation_contract:
  contract_ref: "CREDITBIRD-HR-2026"
  tax_id: "0315397327"
  squad_allocation:
    - role: "Senior Backend (Rust / Golang)"
      fte: 2
      sla_onboarding: "48_hours"
    - role: "Senior Frontend (SvelteKit / React)"
      fte: 1
      sla_onboarding: "48_hours"
    - role: "DevOps & Cloud Security Architect"
      fte: 1
      certifications: ["CKA", "AWS_Solutions_Architect"]
  legal_nda: "B2B Intellectual Property Non-Disclosure Agreement Signed"`,
      events: [
        { from: '@creditbird:talent_pool', to: '@khach_hang:onsite_office', payload: '2 Senior Rust Engineers', status: 'ONBOARD 48H' },
        { from: '@creditbird:talent_pool', to: '@khach_hang:dedicated_team', payload: '1 Senior SvelteKit Dev', status: 'TÍCH HỢP SPRINT' },
        { from: '@creditbird:talent_pool', to: '@khach_hang:cloud_infra', payload: '1 DevOps Cloud Architect', status: 'SECURITY HARDEN' }
      ],
      metrics: [
        { metric: '@it_staff:onboarding', value: 'Bàn giao trong 48 giờ' },
        { metric: '@it_staff:trial_period', value: '14 Ngày thử nghiệm' },
        { metric: '@it_staff:nda_legal', value: '100% Bảo mật sở hữu trí tuệ' }
      ]
    },
    lemyfinest: {
      filename: 'lemyfinest_hvac_iot.yaml',
      title: 'Lemy Finest — Lập Lịch Máy Phun Tinh Dầu HVAC Thông Minh',
      code: `lemyfinest_scent_marketing:
  brand: "LEMY FINEST (lemyfinest.com)"
  hardware: "Diffuser_HVAC_Industrial_Pro_5000"
  space_coverage: "5,000 m² (Hệ thống ống gió điều hòa trung tâm)"
  essential_oil:
    name: "White Tea & Bergamot Luxury Signature"
    certification: "COA & GC-MS Certified 100% Pure Natural"
  iot_scheduler:
    active_days: "Monday - Sunday"
    operating_hours: "07:30 - 21:30"
    burst_cycle: "60s Phun / 120s Nghỉ (Mùi hương lan tỏa đồng đều)"`,
      events: [
        { from: '@lemyfinest:kho_tinh_dau', to: '@may_phun_hvac_toa_nha', payload: 'Tinh dầu COA: 2,500 ml', status: 'TIẾP DẦU ĐỊNH KỲ' },
        { from: '@may_phun_hvac_toa_nha', to: '@khong_gian:sanh_va_van_phong', payload: 'Độ phủ: 5,000 m²', status: 'NANO KHUẾCH TÁN' }
      ],
      metrics: [
        { metric: '@lemyfinest:purity', value: '100% Thiên nhiên COA' },
        { metric: '@lemyfinest:hardware', value: 'Bảo hành 24 tháng' },
        { metric: '@lemyfinest:maintenance', value: 'Bảo dưỡng định kỳ tận nơi' }
      ]
    }
  };

  function copyCode() {
    navigator.clipboard.writeText(scriptExamples[activeTab].code);
    copied = true;
    setTimeout(() => {
      copied = false;
    }, 2000);
  }

  function simulateExecution() {
    isExecuting = true;
    executionResult = null;
    setTimeout(() => {
      isExecuting = false;
      executionResult = {
        traceId: `tr_${Math.random().toString(36).substring(2, 8)}`,
        events: scriptExamples[activeTab].events,
        metrics: scriptExamples[activeTab].metrics
      };
    }, 600);
  }
</script>

<section id="system-sandbox" class="py-20 border-b border-[#e2e8f0] bg-[#f8fafc] scroll-mt-20">
  <div class="strict-grid-container">
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
      
      <!-- Text Description (Left 5 Cols) -->
      <div class="lg:col-span-5">
        <span class="snap-badge mb-3">_KIẾN TRÚC MÃ NGUỒN & HỆ THỐNG / SANDBOX</span>
        <h2 class="section-title text-[#090e1f] mb-4">Trực Quan Hóa Quy Trình Vận Hành Kỹ Thuật</h2>
        <p class="text-[#475569] text-base leading-relaxed mb-6">
          Mỗi phân hệ tại CreditBird được chuẩn hóa bằng mã nguồn tối ưu, cấu hình rõ ràng và tích hợp liên thông: từ động cơ Rust Native của RustSale CRM, định mức sản xuất Kelvot ERP, luồng phái cử nhân sự IT đến cơ chế phun sương Nano HVAC của Lemy Finest.
        </p>

        <!-- Script Features List -->
        <div class="flex flex-col gap-3 font-mono text-xs text-[#475569] mb-8">
          <div class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-[var(--cb-cobalt-500)]"></span>
            <span><strong>RustSale CRM:</strong> Tốc độ microsecond, đồng bộ Zalo / WhatsApp / FB</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-[var(--cb-cobalt-500)]"></span>
            <span><strong>Kelvot ERP:</strong> Định mức BOM đa cấp, kho WMS Barcode, chuẩn VAS</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-[var(--cb-cobalt-500)]"></span>
            <span><strong>Nhân sự IT:</strong> Senior Developers onboard trong 48h, cam kết NDA B2B</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-1.5 h-1.5 rounded-full bg-[var(--cb-cobalt-500)]"></span>
            <span><strong>Lemy Finest:</strong> 100% Tinh dầu COA / GC-MS & Máy phun HVAC 5.000m²</span>
          </div>
        </div>

        <button 
          type="button"
          onclick={simulateExecution}
          disabled={isExecuting}
          class="btn-cta-primary !h-11 cursor-pointer flex items-center gap-2"
        >
          <Play size={14} class={isExecuting ? 'animate-spin' : ''} />
          <span>{isExecuting ? 'Đang kiểm tra luồng...' : 'Chạy mô phỏng hệ thống'}</span>
        </button>
      </div>

      <!-- Code & Execution Sandbox (Right 7 Cols) -->
      <div class="lg:col-span-7">
        <div class="code-card border border-[#cbd5e1] rounded-lg shadow-[0_8px_30px_rgba(0,0,0,0.06)] bg-white overflow-hidden">
          
          <!-- Editor Header with Tabs -->
          <div class="bg-[#f1f5f9] border-b border-[#e2e8f0] px-4 py-2 flex items-center justify-between flex-wrap gap-2">
            
            <!-- File Tabs -->
            <div class="flex items-center gap-1 flex-wrap">
              <button
                type="button"
                onclick={() => { activeTab = 'rustsale'; executionResult = null; }}
                class="flex items-center gap-1.5 px-3 py-1 rounded text-xs font-mono font-medium transition-colors cursor-pointer {activeTab === 'rustsale' ? 'bg-white text-[var(--cb-cobalt-600)] shadow-xs border border-[#cbd5e1]' : 'text-[#64748b] hover:text-[#090e1f]'}"
              >
                <Sparkles size={12} class="text-[var(--cb-cobalt-500)]" />
                <span>rustsale.rs</span>
              </button>

              <button
                type="button"
                onclick={() => { activeTab = 'kelvot'; executionResult = null; }}
                class="flex items-center gap-1.5 px-3 py-1 rounded text-xs font-mono font-medium transition-colors cursor-pointer {activeTab === 'kelvot' ? 'bg-white text-[var(--cb-cobalt-600)] shadow-xs border border-[#cbd5e1]' : 'text-[#64748b] hover:text-[#090e1f]'}"
              >
                <Layers size={12} class="text-[var(--cb-cobalt-500)]" />
                <span>kelvot_mrp.yaml</span>
              </button>

              <button
                type="button"
                onclick={() => { activeTab = 'staffing'; executionResult = null; }}
                class="flex items-center gap-1.5 px-3 py-1 rounded text-xs font-mono font-medium transition-colors cursor-pointer {activeTab === 'staffing' ? 'bg-white text-[var(--cb-cobalt-600)] shadow-xs border border-[#cbd5e1]' : 'text-[#64748b] hover:text-[#090e1f]'}"
              >
                <Users size={12} class="text-[var(--cb-cobalt-500)]" />
                <span>it_staffing.yaml</span>
              </button>

              <button
                type="button"
                onclick={() => { activeTab = 'lemyfinest'; executionResult = null; }}
                class="flex items-center gap-1.5 px-3 py-1 rounded text-xs font-mono font-medium transition-colors cursor-pointer {activeTab === 'lemyfinest' ? 'bg-white text-[var(--cb-cobalt-600)] shadow-xs border border-[#cbd5e1]' : 'text-[#64748b] hover:text-[#090e1f]'}"
              >
                <Wind size={12} class="text-[var(--cb-cobalt-500)]" />
                <span>lemyfinest_iot.yaml</span>
              </button>
            </div>

            <!-- Action Buttons in Tab Header -->
            <div class="flex items-center gap-2">
              <button
                type="button"
                onclick={copyCode}
                class="font-mono text-xs text-[#64748b] hover:text-[#090e1f] flex items-center gap-1 cursor-pointer bg-white px-2 py-1 rounded border border-[#e2e8f0]"
                aria-label="Copy code to clipboard"
              >
                {#if copied}
                  <Check size={13} class="text-[var(--cb-emerald-500)]" />
                  <span class="text-[var(--cb-emerald-500)]">Đã copy</span>
                {:else}
                  <Copy size={13} />
                  <span>Copy</span>
                {/if}
              </button>
            </div>
          </div>

          <!-- Code Window Body -->
          <div class="p-5 font-mono text-[0.8125rem] leading-relaxed bg-[#f8fafc] text-[#1e293b] border-b border-[#e2e8f0] overflow-x-auto max-h-[300px] overflow-y-auto">
            <pre class="m-0 font-mono"><code>{scriptExamples[activeTab].code}</code></pre>
          </div>

          <!-- Interactive Results Container -->
          {#if executionResult}
            <div class="p-5 bg-white font-mono text-xs">
              <div class="flex items-center justify-between border-b border-[#e2e8f0] pb-2 mb-3">
                <div class="flex items-center gap-1.5 text-[var(--cb-emerald-600)] font-semibold">
                  <Check size={14} />
                  <span>MÔ PHỎNG THÀNH CÔNG • TRACE: {executionResult.traceId}</span>
                </div>
                <span class="text-[#94a3b8]">Đồng bộ dữ liệu thời gian thực</span>
              </div>

              <!-- Events Stream -->
              <div class="flex flex-col gap-2 mb-3">
                {#each executionResult.events as ev}
                  <div class="flex flex-wrap items-center justify-between text-[#475569] py-1 border-b border-[#f1f5f9] last:border-0">
                    <div class="flex items-center gap-2">
                      <span class="text-[var(--cb-cobalt-600)]">{ev.from}</span>
                      <span class="text-[#94a3b8]">→</span>
                      <span class="text-[#090e1f] font-medium">{ev.to}</span>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="text-[#64748b]">{ev.payload}</span>
                      <span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[var(--cb-emerald-500)]/10 text-[var(--cb-emerald-600)]">{ev.status}</span>
                    </div>
                  </div>
                {/each}
              </div>

              <!-- System Health & Metrics summary -->
              <div class="pt-2 border-t border-[#e2e8f0] flex flex-wrap gap-4 text-[#64748b]">
                {#each executionResult.metrics as m}
                  <div>
                    <span>{m.metric}:</span>
                    <strong class="text-[#090e1f] ml-1">{m.value}</strong>
                  </div>
                {/each}
              </div>
            </div>
          {/if}

        </div>
      </div>

    </div>
  </div>
</section>
